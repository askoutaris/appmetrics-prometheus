...
namespace AppMetricsPrometheus.Formatters
{
	...
	public class TimerFormatter : BaseFormatter, ITimerFormatter
	{
		public void Write(StreamWriter streamWriter, MetricsContextValueSource metricContext, TimerValueSource metric)
		{
			...
			var fullName = GetFullMetricName(metricContext.Context, metric.MultidimensionalName, metric.Tags, nameof(value.Count));
			streamWriter.WriteLine($"{fullName} {value.Count.ToString(CultureInfo.InvariantCulture)}");
			fullName = GetFullMetricName(metricContext.Context, metric.MultidimensionalName, metric.Tags, nameof(value.Max));
			streamWriter.WriteLine($"{fullName} {value.Max.ToString(CultureInfo.InvariantCulture)}");
			fullName = GetFullMetricName(metricContext.Context, metric.MultidimensionalName, metric.Tags, nameof(value.Mean));
			streamWriter.WriteLine($"{fullName} {value.Mean.ToString(CultureInfo.InvariantCulture)}");
			fullName = GetFullMetricName(metricContext.Context, metric.MultidimensionalName, metric.Tags, nameof(value.Median));
			streamWriter.WriteLine($"{fullName} {value.Median.ToString(CultureInfo.InvariantCulture)}");
			fullName = GetFullMetricName(metricContext.Context, metric.MultidimensionalName, metric.Tags, nameof(value.Min));
			...
			fullName = GetFullMetricName(metricContext.Context, metric.MultidimensionalName, metric.Tags, nameof(value.StdDev));
			streamWriter.WriteLine($"{fullName} {value.StdDev.ToString(CultureInfo.InvariantCulture)}");
			fullName = GetFullMetricName(metricContext.Context, metric.MultidimensionalName, metric.Tags, nameof(value.Sum));
